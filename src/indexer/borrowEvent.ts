...
export async function BorrowEventIndexerZklend(token: any, start: number = -1, end: number = -1) {
    const transferKey = [FieldElement.fromBigInt(hash.getSelectorFromName('Borrowing'))]
    ...
    const tableName = `borrowevent_${token.address}`;
    ...
    if (start !== -1) {
        ...
        console.log("Indexing borrow for", token.name, "current", last_fetched_block)
    }
            ...
            for (let item of message.data.data) {
                ...
                for (let event of block.events) {
                    ...
                    if (token_address_wanted == token_address_spoted) {
                        // console.log(`new borrow detected, hash: ${borrowEvent.hash}`)
                        // console.log(`borrow ${borrowEvent.amount.toString()} of ${borrowEvent.token}`)
                        borrowEventArray.push(borrowEvent)
                    }
                }

                // Push to DB
                for (const event of borrowEventArray) {
                    const client_pg = await pool.connect();
                    const updateQuery = `
